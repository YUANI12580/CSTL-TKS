...
class DataSet(tordata.Dataset):
    def __init__(self, seq_dir, label, seq_type, view, cache, resolution):
        ...
        self.cut_padding = int(float(resolution)/128*20) if int(resolution)==128 else int(float(resolution)/64*10)
    ...
    def __loader__(self, path):
        # return self.img2xarray(path).astype('float32')[:, :, self.cut_padding:-self.cut_padding] / 255.0  # 新增
    ...
    def img2xarray(self, file_path):
        ...
        frame_list = [np.reshape(
            ...
            [self.resolution, self.resolution-self.cut_padding*2, -1])[:, :, 0]
